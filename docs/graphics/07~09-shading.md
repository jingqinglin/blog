...
1. 超采样：对一个像素内的多个点进行纹理采样，再对采样值求平均
    ...
    - 提前获取范围内的平均值
      - 点查询 vs. **（平均）范围查询**
2. Mipmap（显存额外开销 1/3，等比数列求和可算出）：对正方形区域（**屏幕空间一个像素对应纹理像素的正方形区域**）查询结果较好。⭐ **如何查询**第几层——根据两个相邻像素点映射到纹理上的距离长短来确定，见课件 09-P34，视频 50 分钟左右。利用三线性插值计算连续的层数，如 1.8 层。下图的为 mipmap 效果不好的例子，模糊是因为远处的一个像素映射到了非正方形的区域，而这个非正方形的区域由多个 texel 组成，最终查询的值是 texels 求平均后的值，因此会出现模糊：
![](_images/0709-18.png ':class=resizedImage')
