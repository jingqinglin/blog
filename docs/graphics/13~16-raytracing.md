...
- Radiant Energy：电磁辐射的**能量**。$Q[\mathrm{J}=\text{Joule}]$
- Radiant Flux（辐射通量）：单位时间内发射、反射、发射或接收的能量，即**功率**
...
- 单位：$\left[\frac{\mathrm{W}}{\mathrm{sr}}\right]\left[\frac{\operatorname{lm}}{\mathrm{sr}}=\mathrm{cd}=\text{candela}\right]$
