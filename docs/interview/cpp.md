...
再介绍移动语义（`std::move`）。移动语义可以将左值转化为右值引用
...
int main() {
    ...
    // 调用 printInt(int&), i 是左值
    printInt(i);

    // 调用 printInt(int&&), 6 是右值
    printInt(6);

    // 调用 printInt(int&&)，移动语义
    printInt(std::move(i));
}
```
...
假如 reusable 在调用 `foo` 之后就不再被使用了，可以使用移动语义。**移动语义可以将左值转化为右值引用，右值引用可以调用移动构造函数（避免调用拷贝构造函数）**
