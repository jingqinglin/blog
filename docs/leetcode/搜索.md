...
- ⭐ 标记位置的选择很重要。某个点插入队列后应立即标为 1，如下代码：
    ...
    ![](_images/search-1-1.png ':class=image-70')

    否则，搜索到某层时，会把同一层的兄弟节点作为下一层节点。如下图，遍历到第二层，红框内的两个 $0$ 都会把对方作为下一层节点放入队列。

    ![](_images/search-1-2.png ':class=image-30')

    因此，可能会出现这种无意义的路径，导致队列太大，运行时堆内存溢出

    ![](_images/search-1-3.png ':class=image-30') 
...
![](_images/search-3-1.png ':class=image-70')
...
![](_images/search-8-1.png ':class=image-80')
...
![](_images/search-10-2.png ':class=image-30')
